...
| 命令 | 说明 | 示例 |
|------|------|------|
| `启动工作流 [任务名]` | 启动完整6A工作流 | `启动工作流 开发用户管理模块` |
| `查看状态` | 显示当前工作流状态和各阶段进度 | `查看状态` |
| `审批通过` | 审批Approve阶段，启动下一阶段（manual模式） | `审批通过` |
| `暂停工作流` | 暂停当前工作流执行 | `暂停工作流` |
| `恢复工作流` | 恢复暂停的工作流 | `恢复工作流` |
| `终止工作流` | 终止当前工作流 | `终止工作流` |
...
| 命令 | 说明 |
|------|------|
| `使用自动模式` / `开启快速模式` | 设置为自动模式，所有阶段自动流转 |
| `使用人工模式` / `开启严格模式` | 设置为人工模式，Approve阶段需审批 |
...
在manual模式下，仅Approve阶段完成后需等待用户审批，其他阶段（Align、Architect、Atomize、Automate、Assess）自动流转。**必须由用户手动使用`审批通过`命令明确审批Approve阶段**，智能体禁止执行审批命令，否则工作流不会自动流转到Automate阶段。
...
- **manual模式**：仅Approve阶段需用户审批，其他阶段自动流转；`审批通过`命令只能由用户手动执行，智能体禁止执行审批命令
- **审批记录**：审批操作会记录在全局状态文档中，明确标注审批方式（人工/自动）
...
- **auto模式**：
  1. 所有阶段自动流转，无需审批
- **manual模式**：
  1. Align、Architect、Atomize阶段完成后自动流转，无需审批
  2. Approve阶段完成后，SOLO Coder会提示"等待审批"
  3. 用户必须手动运行`审批通过`命令进行审批
  4. 审批通过后，工作流自动进入Automate阶段
  5. Automate、Assess阶段完成后自动流转，无需审批
  6. 如果不进行审批，工作流将保持暂停状态

### 6.7 工作流模式控制

6A工作流支持自动模式和人工模式两种审批模式，用户必须明确设置工作流模式，系统不会默认使用人工模式。
...
- 仅Approve阶段完成后需等待用户审批
- 用户必须使用`审批通过`命令进行审批
- 其他阶段（Align、Architect、Atomize、Automate、Assess）自动流转
- 适用于复杂项目或需要严格质量控制的场景
...
- 必须在启动工作流前明确设置工作流模式
- 模式设置会记录在全局状态文档中
- 所有智能体都会读取全局状态文档确认工作流模式
- 人工模式下仅Approve阶段需使用自然语言审批
...
```
用户：@SOLO-Coder 启动工作流 开发用户管理模块
SOLO-Coder：工作流启动成功，开始Align阶段...
SOLO-Coder：调用@AlgnAgent执行需求对齐
AlgnAgent：正在分析项目上下文...
AlgnAgent：生成ALIGNMENT文档...
AlgnAgent：Align阶段完成，请等待SOLO Coder协调器处理
SOLO-Coder：Align阶段完成，自动流转到Architect阶段
SOLO-Coder：调用@ArchAgent执行架构设计
ArchAgent：正在读取CONSENSUS文档...
ArchAgent：生成DESIGN文档...
ArchAgent：Architect阶段完成，请等待SOLO Coder协调器处理
SOLO-Coder：Architect阶段完成，自动流转到Atomize阶段
SOLO-Coder：调用@AtomAgent执行任务拆解
AtomAgent：正在读取DESIGN文档...
AtomAgent：生成TASK文档...
AtomAgent：Atomize阶段完成，请等待SOLO Coder协调器处理
SOLO-Coder：Atomize阶段完成，自动流转到Approve阶段
SOLO-Coder：调用@AprvAgent执行审批校验
AprvAgent：正在读取ALIGNMENT、CONSENSUS、DESIGN、TASK文档...
AprvAgent：生成REVIEW审查报告...
AprvAgent：Approve阶段完成，请使用 审批通过 命令进行审批
SOLO-Coder：Approve阶段完成，请使用 审批通过 命令进行审批
用户：审批通过
SOLO-Coder：审批通过，自动流转到Automate阶段
SOLO-Coder：调用@AutmAgent执行编码执行
AutmAgent：正在读取TASK文档...
AutmAgent：正在实现代码...
AutmAgent：生成ACCEPTANCE文档...
AutmAgent：Automate阶段完成，请等待SOLO Coder协调器处理
SOLO-Coder：Automate阶段完成，自动流转到Assess阶段
SOLO-Coder：调用@AsseAgent执行评估验收
AsseAgent：正在读取ACCEPTANCE文档...
AsseAgent：生成FINAL、TODO、VERIFICATION文档...
AsseAgent：Assess阶段完成，工作流已完成
SOLO-Coder：6A工作流已完成，项目交付完成！
```
...
SOLO-Coder：调用@AprvAgent执行审批校验
AprvAgent：正在读取ALIGNMENT、CONSENSUS、DESIGN、TASK文档...
AprvAgent：发现架构设计与需求存在冲突，生成REVIEW审查报告...
AprvAgent：审查结论：退回整改，目标阶段Architect
SOLO-Coder：Approve审查退回，调度@ArchAgent执行整改
ArchAgent：读取整改指令文档RECTIFY_ARCHITECT中的整改要求...
ArchAgent：更新DESIGN文档...
ArchAgent：整改完成，更新状态文档
SOLO-Coder：Architect整改完成，重新启动Approve校验
AprvAgent：重新读取所有前序文档...
AprvAgent：审查结论：通过
SOLO-Coder：Approve阶段完成，请使用 审批通过 命令进行审批
```
...
SOLO-Coder：调用@AlgnAgent执行需求对齐
AlgnAgent：正在分析项目上下文...
AlgnAgent：发现需求歧义，生成QUESTIONS_ALIGN问题报告...
AlgnAgent：自主选择推荐方案（标注为"自动决策"），生成ANSWERS_ALIGN文档...
AlgnAgent：基于自动决策结果，生成CONSENSUS文档...
AlgnAgent：Align阶段完成，自动流转到Architect阶段
```
...
- **高效执行**：适合需求明确、技术方案成熟的场景
- **减少干预**：用户只需提出需求，无需持续跟进
- **自动化程度高**：机器自动审批流转，快速交付
- **适合快速迭代**：适用于原型开发、POC验证等场景
